...
export function PythonModuleProgress() {
  ...
  useEffect(() => {
    // In a real app, we fetch from /api/progress/python
    // For now, mock a progress fetch
    const fetchProgress = async () => {
      try {
        // const res = await fetch('/api/progress/python');
        // const data = await res.json();
        const mockData = { completed: 0, total: 20 };
  ...
  return (
    <div className="w-full max-w-md mt-8 bg-black/20 backdrop-blur-xl border border-border rounded-2xl p-5 shadow-2xl">
      <div className="flex items-center justify-between mb-4">
        ...
        <span className="text-xl font-display font-bold text-cyan-400">{Math.round(progress)}%</span>
      </div>
      
      <Progress value={progress} className="h-2 mb-6 bg-white/10" />
      
      <Link href="/learn/python/python-intro" className="block w-full">
        <Button className="w-full bg-cyan-500 hover:bg-cyan-400 text-black font-bold shadow-[0_0_15px_rgba(34,211,238,0.3)] transition-all flex items-center justify-center gap-2">
