...
import { CheckCircle2, PlayCircle, BookOpen, Code2, MonitorPlay, Clock, Zap, X, Lock } from "lucide-react";
        ...
        {lessons.map((lesson: any) => {
          ...
          return (
            <motion.div 
              ...
              variants={item}
              className={cn(
                "group flex flex-col sm:flex-row sm:items-center justify-between p-4 rounded-xl border bg-black/20 backdrop-blur-sm transition-colors",
                isLocked ? "border-border/50 opacity-50 cursor-not-allowed" : "border-border hover:bg-white/5 cursor-pointer"
              )}
              onClick={() => !isLocked && setSelectedLesson(lesson)}
            >
              <div className="flex items-center gap-4 mb-4 sm:mb-0">
                <div className={cn(
                  "w-10 h-10 rounded-full flex items-center justify-center shrink-0 border shadow-inner transition-colors",
                  loading ? "bg-surface-deep border-slate-700 text-slate-500" :
                  status === "completed" ? "bg-emerald-500/10 border-emerald-500/20 text-emerald-400" :
                  status === "in_progress" ? "bg-indigo-500/10 border-indigo-500/20 text-indigo-400" :
                ...
                <div>
                  <div className="flex items-center gap-3">
                    <h4 className={cn(
                      "font-semibold",
                      status === "completed" ? "text-slate-300" : "text-white group-hover:text-cyan-300 transition-colors"
                    )}>
                      {lesson.title}
                    </h4>
              ...
              <Button 
                ...
                className={cn(
                  ...
                  loading ? "border-slate-800 text-slate-600 bg-transparent" :
                  status === "completed" ? "border-emerald-500/30 text-emerald-400 hover:bg-emerald-500/10" :
                  status === "in_progress" ? "border-indigo-500/50 bg-indigo-500/10 text-indigo-300 hover:bg-indigo-500 hover:text-white" :
                  "border-white/20 text-slate-300 hover:border-cyan-500 hover:text-cyan-400 hover:bg-cyan-500/10"
                )}
              >
                {loading ? "Loading..." : isLocked ? "Locked" : status === "completed" ? "Review" : status === "in_progress" ? "Resume" : "Select"}
              </Button>
            </motion.div>
          );
        })}
